 ...
 */
const getResourcePath = lodash_1.default.memoize(async function getResourcePath(relPath) {
...
async function getAndroidPlatformAndPath(sdkRoot) {
    ...
    for (const propsPath of propsPaths) {
        ...
        if (!match) {
            logger_js_1.default.warn(`Cannot read the SDK version from '${propsPath}'. Skipping '${platform}'`);
            continue;
        }
    ...
    if (lodash_1.default.isEmpty(platformsMapping)) {
        logger_js_1.default.warn(`Found zero platform folders at '${path_1.default.resolve(sdkRoot, 'platforms')}'. ` +
    ...
    logger_js_1.default.debug(`Found the most recent Android platform: ${JSON.stringify(result)}`);
    return result;
}
...
async function unzipFile(zipPath, dstRoot = path_1.default.dirname(zipPath)) {
    logger_js_1.default.debug(`Unzipping '${zipPath}' to '${dstRoot}'`);
    await support_1.zip.assertValidZip(zipPath);
    await support_1.zip.extractAllTo(zipPath, dstRoot);
    logger_js_1.default.debug('Unzip successful');
}
...
/** @type {() => Promise<string>} */
const getJavaHome = lodash_1.default.memoize(async function getJavaHome() {
...
/** @type {() => Promise<string>} */
const getJavaForOs = lodash_1.default.memoize(async function getJavaForOs() {
    let javaHome;
    let errMsg;
    try {
...
exports.isInDozingMode = isInDozingMode;
/*
...
function buildStartCmd(startAppOptions, apiLevel) {
    const { user, waitForLaunch, pkg, activity, action, category, stopApp, flags, } = startAppOptions;
    const cmd = ['am', 'start'];
...
const getSdkToolsVersion = lodash_1.default.memoize(async function getSdkToolsVersion() {
    const androidHome = process.env.ANDROID_HOME;
    if (!androidHome) {
        throw new Error('ANDROID_HOME environment variable is expected to be set');
    }
    const propertiesPath = path_1.default.resolve(androidHome, 'tools', 'source.properties');
    if (!await support_1.fs.exists(propertiesPath)) {
        logger_js_1.default.warn(`Cannot find ${propertiesPath} file to read SDK version from`);
        return null;
    }
    const propertiesContent = await support_1.fs.readFile(propertiesPath, 'utf8');
    ...
    logger_js_1.default.warn(`Cannot parse "Pkg.Revision" value from ${propertiesPath}`);
    return null;
});
 ...
 */
const getBuildToolsDirs = lodash_1.default.memoize(async function getBuildToolsDirs(sdkRoot) {
    ...
    try {
        buildToolsDirs = buildToolsDirs
            ...
            .sort((a, b) => semver_1.default.rcompare(a[0], b[0]))
            .map((pair) => pair[1]);
    }
    catch (err) {
        logger_js_1.default.warn(`Cannot sort build-tools folders ${JSON.stringify(buildToolsDirs.map((dir) => path_1.default.basename(dir)))} ` +
            `by semantic version names.`);
        logger_js_1.default.warn(`Falling back to sorting by modification date. Original error: ${err.message}`);
        /** @type {[number, string][]} */
    ...
    logger_js_1.default.info(`Found ${buildToolsDirs.length} 'build-tools' folders under '${sdkRoot}' (newest first):`);
    for (let dir of buildToolsDirs) {
        logger_js_1.default.info(`    ${dir}`);
    }
...
const extractMatchingPermissions = function (dumpsysOutput, groupNames, grantedState = null) {
    ...
    logger_js_1.default.debug(`Retrieved ${support_1.util.pluralize('permission', filteredResult.length, true)} ` +
...
function buildInstallArgs(apiLevel, options = {}) {
    ...
    if (options.grantPermissions) {
        if (apiLevel < 23) {
            logger_js_1.default.debug(`Skipping permissions grant option, since ` +
...
function parseManifest(manifest) {
    ...
    for (const activity of [
        ...
        if (activity.enabled === false || lodash_1.default.isEmpty(activity.intentFilters)) {
            continue;
        }
        for (const { actions, categories } of activity.intentFilters) {
            if (lodash_1.default.isEmpty(actions) || lodash_1.default.isEmpty(categories)) {
                continue;
            }
...
function parseAaptStrings(rawOutput, configMarker) {
    ...
    for (const line of rawOutput.split(os_1.default.EOL)) {
...
function parseAapt2Strings(rawOutput, configMarker) {
    const allLines = rawOutput.split(os_1.default.EOL);
    function extractContent(startIdx) {
        ...
        while (idx < allLines.length) {
            ...
            if (terminationCharMatch) {
                ...
                return [
                    `${result}\\n${lodash_1.default.trimStart(allLines[idx].substring(0, terminationCharPos))}`,
                    idx,
                ];
            }
            if (idx > startIdx) {
...
async function formatConfigMarker(configsGetter, desiredMarker, defaultMarker) {
    ...
    logger_js_1.default.debug(`Resource configurations: ${JSON.stringify(configs)}`);
    ...
    if (configMarker.toLowerCase().startsWith('en')
        ...
        logger_js_1.default.debug(`Resource configuration name '${configMarker}' is unknown. ` +
            `Replacing it with '${defaultMarker}'`);
        configMarker = defaultMarker;
    }
    else {
        logger_js_1.default.debug(`Selected configuration: '${configMarker}'`);
    }
    return configMarker;
}
...
async function getAndroidPrefsRoot() {
    ...
    if (location) {
        logger_js_1.default.warn(`The value of the ANDROID_EMULATOR_HOME environment variable '${location}' is not an existing directory`);
    }
    const home = process.env.HOME || process.env.USERPROFILE;
    if (home) {
        location = path_1.default.resolve(home, '.android');
    }
    if (!await dirExists(location ?? '')) {
        logger_js_1.default.debug(`Android config root '${location}' is not an existing directory`);
        return null;
    }
    return location ?? null;
}
